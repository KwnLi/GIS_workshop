---
title: "Conduct buffer analysis (with R)"
output:
  html_document:
    toc: true
    toc_depth: 3
...
## 4. Extracting land cover areas in a buffer around each point
...
lf500 <- terra::extract(lf.pr, farms500, list = TRUE, touches = TRUE, xy = TRUE) %>%
  ...
  bind_cols(site = farms500$siteName)  # re-add the site name from the buffer layer

lfcrop <- terra::crop(lf.pr, farms500)

```

## 5. Examining the buffer data
